...
@TokenIndexer.register("pointer_generator")
class PointerGeneratorTokenIndexer(TokenIndexer[int]):
    """
    ...
    def tokens_to_indices(self,
        ...
        oov_index = vocabulary.get_vocab_size()
        oov_dict = {}
        for token in itertools.chain(self._start_tokens, tokens, self._end_tokens):
            if getattr(token, 'text_id', None) is not None:
                ...
                if index == vocabulary.get_token_index(vocabulary._oov_token, self.namespace):
                    if text in oov_dict:
                        index = oov_dict[text]
                    else:
                        index = oov_index
